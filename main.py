...
import ioimport mathfrom datetime import datetime, timedeltafrom html import escape

import pandas as pdimport requestsimport streamlit as st

def parse_mac_datetime(value):if isinstance(value, datetime):return values = str(value).strip()for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%dT%H:%M"):try:return datetime.strptime(s, fmt)except Exception:passtry:return datetime.fromisoformat(s.replace("Z", ""))except Exception:return datetime.now()
...
def get_secret_value(name, default=""):try:return st.secrets.get(name, default)except Exception:return default

def get_app_api_key():user_key = str(st.session_state.get("user_api_key", "")).strip()if user_key:return user_keyreturn str(get_secret_value("ODDS_API_KEY", "")).strip()

def api_key_panel():with st.sidebar:st.markdown("### 🔑 API Key Girişi")
...
def require_api_key():if not get_app_api_key():st.warning("Devam etmek için sol menüden ODDS API KEY girmen gerekiyor ⚠️")st.stop()

def limit_for_free(items, free_limit=999999):# Üyelik sistemi kaldırıldı. Artık sınırlama yok.return list(items or [])

def legal_notice_top():st.markdown("""⚠️ Yasal Uyarı: Bu platform yalnızca istatistiksel analizler, geçmiş veri karşılaştırmaları ve yapay zekâ destekli tahminler sunar.Kesin kazanç garantisi verilmez. Bahis oynamak risk içerir ve bağımlılık oluşturabilir.""",unsafe_allow_html=True,)

def legal_sidebar_sections():"""Sidebar içinde disclaimer ve kullanım şartları."""with st.sidebar:st.markdown("---")
...
1. Hizmet TanımıBu platform, spor karşılaşmalarına ilişkin istatistiksel analizler ve yapay zekâ destekli tahminler sunar.

2. Sorumluluk ReddiPlatformda yer alan hiçbir içerik kesin kazanç garantisi vermez. Kullanıcılar, elde ettikleri verileri kendi riskleri doğrultusunda değerlendirir.

3. Bahis Hizmeti SunulmamasıBu platform bir bahis sitesi değildir. Kullanıcılara doğrudan bahis oynama imkânı sunulmaz ve herhangi bir bahis kuruluşu ile resmi bir bağlantısı bulunmaz.

4. Kullanıcı SorumluluğuKullanıcılar, platformu kullanırken yürürlükteki yasalara uymakla yükümlüdür.

5. Hizmet DeğişikliğiPlatform, hizmet içeriğini önceden bildirmeksizin değiştirme hakkını saklı tutar.""")

def legal_footer():"""Sayfanın en altında kısa hukuki footer."""st.markdown("""---Yasal Uyarı: Bu platform yalnızca istatistiksel analiz ve yapay zekâ destekli tahminler sunar.Kesin kazanç garantisi verilmez. Kullanıcılar kararlarını kendi sorumluluğunda verir.Bu platform üzerinden doğrudan bahis oynanmaz. Bahis oynamak risk içerir ve maddi kayıplara yol açabilir.""", unsafe_allow_html=True)



APP_SCHEMA_VERSION = 19if st.session_state.get("app_schema_version") != APP_SCHEMA_VERSION:st.session_state.clear()st.session_state["app_schema_version"] = APP_SCHEMA_VERSION

st.markdown("""

""", unsafe_allow_html=True)

api_key_panel()legal_sidebar_sections()legal_notice_top()

def format_tr_date(d):aylar = {1: "Ocak", 2: "Şubat", 3: "Mart", 4: "Nisan", 5: "Mayıs", 6: "Haziran",7: "Temmuz", 8: "Ağustos", 9: "Eylül", 10: "Ekim", 11: "Kasım", 12: "Aralık"}gunler = {0: "Pazartesi", 1: "Salı", 2: "Çarşamba", 3: "Perşembe",4: "Cuma", 5: "Cumartesi", 6: "Pazar"}return f"{d.day} {aylar[d.month]} {d.year} {gunler[d.weekday()]}"

def dinamik_min_mac(tolerans: float) -> int:if tolerans <= 0.02:return 1elif tolerans <= 0.05:return 3elif tolerans <= 0.08:return 5elif tolerans <= 0.12:return 10return 20

def sample_factor_hesapla(sample: int, tolerans: float) -> float:if tolerans <= 0.02:hedef = 5elif tolerans <= 0.05:hedef = 10elif tolerans <= 0.08:hedef = 15elif tolerans <= 0.12:hedef = 25else:hedef = 40return min(0.75 + 0.25 * (sample / max(hedef, 1)), 1.0)

def tolerans_rehberi(tolerans: float):min_mac = dinamik_min_mac(tolerans)if tolerans <= 0.02:yorum = "Çok dar filtre. Az ama çok yakın oranlı örnekler gelir."elif tolerans <= 0.05:yorum = "Dar filtre. Örnek az olabilir ama eşleşme kalitesi yüksektir."elif tolerans <= 0.08:yorum = "Dengeli filtre. Hem kalite hem örnek sayısı dengeli."elif tolerans <= 0.12:yorum = "Biraz geniş filtre. Veri artar, benzerlik biraz düşer."else:yorum = "Geniş filtre. Sonuçlar daha genel davranabilir."return {"onerilen_tolerans": "0.08 - 0.10","onerilen_min_mac": min_mac,"yorum": yorum,}

def guven_metni(sample: int, tolerans: float):min_mac = dinamik_min_mac(tolerans)if sample >= max(20, min_mac * 3):return "Çok Sağlam", "#27ae60"if sample >= max(10, min_mac * 2):return "Sağlıklı", "#2ecc71"if sample >= min_mac:return "Kullanılabilir", "#f39c12"return "Riskli", "#e74c3c"

def guven_renk(pct: int):if pct >= 70:return "#27ae60", "badge-yuksek", "Yüksek Güven"if pct >= 55:return "#e67e22", "badge-orta", "Orta Güven"return "#e74c3c", "badge-dusuk", "Düşük Güven"

def risk_seviyesi(pct: int, flip_p: float):if pct >= 70 and flip_p < 0.15:return "DÜŞÜK", "risk-dusuk"if pct >= 55:return "ORTA", "risk-orta"return "YÜKSEK", "risk-yuksek"

def tahmini_skor(b: pd.DataFrame, ms_mod: str):eg = math.floor(b["FTHG"].mean() + 0.5) if not b.empty else 1dg = math.floor(b["FTAG"].mean() + 0.5) if not b.empty else 1if ms_mod == "H" and eg <= dg:eg = dg + 1if ms_mod == "A" and dg <= eg:dg = eg + 1return eg, dg

def mac_tipi(h: float, a: float):if abs(h - a) <= 0.50:return "Dengeli"if h < 2.0 or a < 2.0:return "Favori"return "Sürpriz Açık"

def gol_profili(avg_goal: float):if avg_goal < 2.2:return "Düşük Gollü"if avg_goal < 3.0:return "Dengeli"return "Yüksek Gollü"

def fake_confidence_duzelt(conf_prob: float, sample: int, tolerans: float):carpan = 1.0if tolerans <= 0.05 and sample < 10 and conf_prob > 0.80:carpan *= 0.75elif tolerans <= 0.08 and sample < 8 and conf_prob > 0.75:carpan *= 0.82return conf_prob * carpan, carpan < 1.0

@st.cache_data(ttl=86400)def futbol_veri_motoru(sezonlar):if not sezonlar:return pd.DataFrame()
...
@st.cache_data(ttl=1800, show_spinner=False)def bulten_cek(key, kodlar, t):secret_key = get_app_api_key()if secret_key:key = secret_keyif not key:st.error("ODDS_API_KEY bulunamadı. Streamlit Cloud > Settings > Secrets içine ODDS_API_KEY eklemelisin.")return pd.DataFrame()res = []
...
def fmt_odd(odd):if odd is None:return ""try:return f"{float(odd):.2f}"except Exception:return ""

def pct100(v):try:return max(0, min(100, int(round(float(v)))))except Exception:return 0

def skoru_tahmine_uydur(eg, dg, ana_label, ms_mod):ana = str(ana_label)ms_mod = str(ms_mod)
...
def ai_kart_yorumlari(t, m):ana = t.get("ana_label", "")guven = int(t.get("ana_p", 0))puan = float(t.get("playable_score", guven))canli = t.get("canli_label", "İlk 15 dk izle")
...
def ai_yorum_uret(t):ana = t.get("ana_label", "")guven = int(t.get("ana_p", 0))puan = float(t.get("playable_score", guven))ornek = int(t.get("ornek", 0))mac_tipi_txt = t.get("match_type", "")gol_profili = t.get("goal_profile", "")combo = t.get("combo_label", "")canli = t.get("canli_label", "")
...
def build_top3_coupon(indexed_items, mode="best_favorites"):candidates = []
...
def market_label_to_odd(m_row, label):if not isinstance(m_row, dict):try:m_row = m_row.to_dict()except Exception:passif label == "MS 1":return m_row.get("h")if label == "MS 2":return m_row.get("a")if label == "Beraberlik":return m_row.get("b")return None

def hesapla(b_df, m_row, tolerans):rehber = tolerans_rehberi(float(tolerans))onerilen_min_mac = dinamik_min_mac(float(tolerans))
...
def kombo_tahmini_oran(label, ana_odd):try:ana_odd = float(ana_odd or 1.0)except Exception:ana_odd = 1.0
...
def top10_market_adaylari(t):"""Top 10 için gerçek multi-market aday havuzu.Sadece MS'e kilitlenmez; MS / Alt-Üst / KG / İlk Yarı / Kombo marketlerini aynı havuza alır.Non-MS marketlere bilinçli bonus verir ki Top 10 sadece MS1-MS2 dolmasın."""adaylar = []
...
def mac_key(m):"""Aynı maçı tekilleştirmek için güvenli key üretir."""try:if hasattr(m, "to_dict"):m = m.to_dict()except Exception:passtry:zaman = m.get("zaman", "")if hasattr(zaman, "strftime"):zaman = zaman.strftime("%Y-%m-%d %H:%M")return f"{m.get(lig,)}::{m.get(ev,)}::{m.get(dep,)}::{zaman}"except Exception:return str(m)def gunun_en_iyi_10_uret(gecmis_df, bulten_df, min_ornek=1, limit=10):"""Günün Top 10 listesini seçili hassasiyete bağlamaz.Her maç için 0.00 - 0.10 arası toleransları dener.MS / Alt-Üst / KG / İlk Yarı / Kombo adayları arasından maçın en iyi marketini seçer.API kullanmaz; sadece mevcut bülten + geçmiş veri üzerinden hesaplama yapar."""top_toleranslar = [0.00, 0.02, 0.04, 0.06, 0.08, 0.10]adaylar = []
...
for key, default in [("final_list", []),("detay_idx", None),("detay_item", None),("top10_list", []),("filtre", "tumu"),("kupona", []),("coupon_popup_open", False),("last_gecmis_df", None),("last_bulten_df", None),]:if key not in st.session_state:st.session_state[key] = default

FUTBOL_LIGLERI = {"AVRUPA KUPALARI": {"Şampiyonlar Ligi": "soccer_uefa_champs_league","Avrupa Ligi": "soccer_uefa_europa_league","Konferans Ligi": "soccer_uefa_europa_conference_league",},"TÜRKİYE": {"Süper Lig": "soccer_turkey_super_league",},"İNGİLTERE": {"Premier League": "soccer_epl","Championship": "soccer_efl_champ","League 1": "soccer_england_league1","League 2": "soccer_england_league2","FA Cup": "soccer_fa_cup","EFL Cup": "soccer_england_efl_cup",},"İSPANYA": {"La Liga": "soccer_spain_la_liga","La Liga 2": "soccer_spain_segunda_division","Copa del Rey": "soccer_spain_copa_del_rey",},"ALMANYA": {"Bundesliga": "soccer_germany_bundesliga","Bundesliga 2": "soccer_germany_bundesliga2","DFB-Pokal": "soccer_germany_dfb_pokal",},"İTALYA": {"Serie A": "soccer_italy_serie_a","Serie B": "soccer_italy_serie_b","Coppa Italia": "soccer_italy_coppa_italia",},"FRANSA": {"Ligue 1": "soccer_france_ligue_one","Ligue 2": "soccer_france_ligue_two","Coupe de France": "soccer_france_coupe_de_france",},"AVRUPA VALUE": {"Hollanda": "soccer_netherlands_eredivisie","Belçika": "soccer_belgium_first_div","Portekiz": "soccer_portugal_primeira_liga","İskoçya": "soccer_spl","Danimarka": "soccer_denmark_superliga","Avusturya": "soccer_austria_bundesliga","İsviçre": "soccer_switzerland_superleague","İsveç": "soccer_sweden_allsvenskan","Norveç": "soccer_norway_eliteserien","Polonya": "soccer_poland_ekstraklasa","Finlandiya": "soccer_finland_veikkausliiga","İrlanda": "soccer_league_of_ireland","Yunanistan": "soccer_greece_super_league",},"DÜNYA VALUE": {"MLS": "soccer_usa_mls","Brezilya Serie A": "soccer_brazil_campeonato","Arjantin Primera": "soccer_argentina_primera_division","Japonya J League": "soccer_japan_j_league","Meksika Liga MX": "soccer_mexico_ligamx","Güney Kore K League 1": "soccer_korea_kleague1","Şili Primera": "soccer_chile_campeonato",},}

LEAGUE_EMOJIS = {"Şampiyonlar Ligi": "🏆","Avrupa Ligi": "🟠","Konferans Ligi": "🟢","Süper Lig": "🇹🇷","Premier League": "🏴","Championship": "🏴","League 1": "🏴","League 2": "🏴","FA Cup": "🏴","EFL Cup": "🏴","La Liga": "🇪🇸","La Liga 2": "🇪🇸","Copa del Rey": "🇪🇸","Bundesliga": "🇩🇪","Bundesliga 2": "🇩🇪","DFB-Pokal": "🇩🇪","Serie A": "🇮🇹","Serie B": "🇮🇹","Coppa Italia": "🇮🇹","Ligue 1": "🇫🇷","Ligue 2": "🇫🇷","Coupe de France": "🇫🇷","Hollanda": "🇳🇱","Belçika": "🇧🇪","Portekiz": "🇵🇹","İskoçya": "🏴","Danimarka": "🇩🇰","Avusturya": "🇦🇹","İsviçre": "🇨🇭","İsveç": "🇸🇪","Norveç": "🇳🇴","Polonya": "🇵🇱","Finlandiya": "🇫🇮","İrlanda": "🇮🇪","Yunanistan": "🇬🇷","MLS": "🇺🇸","Brezilya Serie A": "🇧🇷","Arjantin Primera": "🇦🇷","Japonya J League": "🇯🇵","Meksika Liga MX": "🇲🇽","Güney Kore K League 1": "🇰🇷","Şili Primera": "🇨🇱",}

def lig_etiketi(isim: str) -> str:emoji = LEAGUE_EMOJIS.get(isim, "⚽")return f"{emoji} {isim}"

def tum_lig_listesi():rows = []for kat, ligler in FUTBOL_LIGLERI.items():for isim, kod in ligler.items():rows.append({"kategori": kat,"isim": isim,"kod": kod,"label": lig_etiketi(isim),})return rows

def filtrelenmis_lig_listesi(arama_text: str):ligler = tum_lig_listesi()if not arama_text:return ligler
...
KARLI_LIG_PRESETLERI = {# Popup içindeki tek hızlı filtre: Kararlı çekirdek + kârlı/value ligler birlikte seçilir."cekirdek_value": [# Kararlı çekirdek"soccer_epl","soccer_efl_champ","soccer_spain_la_liga","soccer_spain_segunda_division","soccer_italy_serie_a","soccer_germany_bundesliga","soccer_germany_bundesliga2","soccer_france_ligue_one","soccer_turkey_super_league","soccer_netherlands_eredivisie","soccer_norway_eliteserien","soccer_usa_mls","soccer_switzerland_superleague","soccer_uefa_champs_league","soccer_uefa_europa_league","soccer_uefa_europa_conference_league",# Kârlı / value ek ligler"soccer_portugal_primeira_liga","soccer_belgium_first_div","soccer_austria_bundesliga","soccer_denmark_superliga","soccer_sweden_allsvenskan","soccer_finland_veikkausliiga",],}

def tum_lig_kodlari():return [kod for ligler in FUTBOL_LIGLERI.values() for kod in ligler.values()]

def init_league_states():for _, ligler in FUTBOL_LIGLERI.items():for , kod in ligler.items():item_key = f"cb{kod}"if item_key not in st.session_state:st.session_state[item_key] = False



def set_leagues(selected_codes):secili = set(selected_codes)for kod in tum_lig_kodlari():st.session_state[f"cb_{kod}"] = kod in secili



def clear_leagues():set_leagues([])

def toggle_leagues(selected_codes):secili = set(selected_codes)tumu_aktif = all(st.session_state.get(f"cb_{kod}", False) for kod in secili) if secili else Falseif tumu_aktif:for kod in secili:st.session_state[f"cb_{kod}"] = Falseelse:set_leagues(selected_codes)

def sonraki_hafta_gunu(baslangic_tarihi, hedef_weekday: int):gun_farki = (hedef_weekday - baslangic_tarihi.weekday()) % 7return baslangic_tarihi + timedelta(days=gun_farki)

def tarih_secimine_gore_date(secim: str, bugun_tarih, ozel_tarih):if secim == "Bugün":return bugun_tarihif secim == "Yarın":return bugun_tarih + timedelta(days=1)if secim == "2 gün sonra":return bugun_tarih + timedelta(days=2)if secim == "3 gün sonra":return bugun_tarih + timedelta(days=3)return ozel_tarih

def mac_canli_durumu(mac_zamani):now = datetime.now()if now < mac_zamani:return "Başlamamış"if now <= mac_zamani + timedelta(hours=2, minutes=15):return "Canlı"return "Bitti"

def mac_durum_badge(mac_zamani):durum = mac_canli_durumu(mac_zamani)if durum == "Canlı":return "#16a34a", "CANLI"if durum == "Başlamamış":return "#2563eb", "YAKINDA"return "#64748b", "BİTTİ"

init_league_states()secili_kodlar = []

ÜST KONTROL BAR

bugun = datetime.now().date()st.markdown('', unsafe_allow_html=True)with st.expander("🔑 API Ayarları", expanded=False):API_KEY = get_app_api_key()if API_KEY:st.success("API key sistemde tanımlı ✅")else:st.error("ODDS_API_KEY Streamlit Secrets içinde tanımlı değil.")st.markdown('', unsafe_allow_html=True)

st.markdown("""

""", unsafe_allow_html=True)

def selected_league_codes():return [lig['kod'] for lig in tum_lig_listesi() if st.session_state.get(f"cb_{lig['kod']}", False)]

if 'date_mode' not in st.session_state:st.session_state['date_mode'] = 'Bugün'if 'special_date' not in st.session_state:st.session_state['special_date'] = bugun

st.markdown("""

bar1, bar2, bar3, bar4, bar5, bar6 = st.columns([2.6, 2.0, 1.4, 1.8, 1.8, 2.1], gap='small')

with bar1:st.markdown('', unsafe_allow_html=True)st.markdown('Tarih ve Lig Seçimi', unsafe_allow_html=True)secili_tarih = tarih_secimine_gore_date(st.session_state.get('date_mode', 'Bugün'),bugun,st.session_state.get('special_date', bugun))popover_title = f"⚽ Tarih ve Lig Seçimi · {len(selected_league_codes())} lig seçili · {format_tr_date(secili_tarih)}"with st.popover(popover_title, use_container_width=True):left_col, right_col = st.columns([1.0, 3.2], gap='medium')with left_col:st.markdown('Tarih Seçimi', unsafe_allow_html=True)date_mode = st.radio('Tarih modu',options=['Bugün', 'Yarın', '2 gün sonra', '3 gün sonra', 'Özel Tarih'],index=['Bugün', 'Yarın', '2 gün sonra', '3 gün sonra', 'Özel Tarih'].index(st.session_state.get('date_mode', 'Bugün')),key='date_mode',label_visibility='collapsed')if date_mode == 'Özel Tarih':st.date_input('Özel tarih', value=st.session_state.get('special_date', bugun), key='special_date')
...
with bar2:st.markdown('', unsafe_allow_html=True)st.markdown('Sezonlar', unsafe_allow_html=True)yillar = st.multiselect('Sezonlar',options=['2122', '2223', '2324', '2425', '2526'],default=['2122', '2223', '2324', '2425', '2526'],label_visibility='collapsed',key='top_seasons')st.markdown('', unsafe_allow_html=True)

with bar3:st.markdown('', unsafe_allow_html=True)st.markdown('Min. Örnek Sayısı', unsafe_allow_html=True)min_ornek = st.number_input('Min. Örnek Sayısı', min_value=1, value=1, label_visibility='collapsed', key='top_min_ornek')st.markdown('', unsafe_allow_html=True)

with bar4:st.markdown('', unsafe_allow_html=True)st.markdown('Oran Hassasiyeti', unsafe_allow_html=True)TOLERANS = st.slider('Oran Hassasiyeti', 0.00, 0.30, 0.08, step=0.01, label_visibility='collapsed', key='top_tol')st.markdown(f"{TOLERANS:.2f}", unsafe_allow_html=True)st.markdown('', unsafe_allow_html=True)

with bar5:st.markdown('', unsafe_allow_html=True)st.markdown('Oynanılabilir / Canlı', unsafe_allow_html=True)oynanabilir_esik = st.selectbox('Oynanılabilir eşik',options=[0, 55, 60, 65, 70, 75],index=2,format_func=lambda x: 'Tümü' if x == 0 else f'Güven ≥ %{x}',label_visibility='collapsed',key='oynanabilir_esik')canli_filtre = st.selectbox('Canlı filtre',options=['Tümü', 'Canlı', 'Başlamamış', 'Bitti'],index=0,label_visibility='collapsed',key='canli_filtre')st.markdown('', unsafe_allow_html=True)

secili_kodlar = selected_league_codes()with bar6:st.markdown('', unsafe_allow_html=True)st.markdown('Analiz', unsafe_allow_html=True)analiz_btn = st.button('▶ ANALİZİ BAŞLAT', use_container_width=True, type='primary', key='analiz_baslat_btn')if st.button('🎫 Kuponlarım', use_container_width=True, key='toggle_coupon_popup'):st.session_state.coupon_popup_open = Truest.rerun()if 'son_analiz' in st.session_state:st.markdown(f"Son analiz: {st.session_state.son_analiz}Toplam maç: {st.session_state.get('toplam_mac',0)}",unsafe_allow_html=True,)st.markdown('', unsafe_allow_html=True)

rehber = tolerans_rehberi(TOLERANS)st.markdown(f"""

if analiz_btn:if not API_KEY or not secili_kodlar:st.error("⚠️ API Key ve en az bir lig seçin.")else:with st.spinner("📊 Veriler çekiliyor ve analiz ediliyor..."):gecmis = futbol_veri_motoru(tuple(yillar))bulten = bulten_cek(API_KEY, secili_kodlar, secili_tarih)st.session_state.last_gecmis_df = gecmisst.session_state.last_bulten_df = bulten
...
if st.session_state.detay_item is not None or st.session_state.detay_idx is not None:if st.session_state.detay_item is not None:item = st.session_state.detay_itemelse:idx = st.session_state.detay_idxitem = st.session_state.final_list[idx]m, t, b_det = item["m"], item["t"], item["b"]
...
hc1, hc2 = st.columns([6, 1])with hc1:st.markdown(f"""ANA MAÇ EKRANI{format_tr_date(secili_tarih)}""", unsafe_allow_html=True)
...
with hc2:if fl:st.markdown(f"""{len(fl)}MAÇ BULUNDU""", unsafe_allow_html=True)
...
st.markdown("", unsafe_allow_html=True)st.info("🧠 Sade mod aktif: Auto kupon builder ve 30 günlük kasa planı kaldırıldı. Tolerans değişiminde sadece analiz yenilenir; yeni API çağrısı için tekrar maç yüklemen gerekir.")

if not fl:st.markdown("""⚡Analizi BaşlatınAPI ayarlarını açıp anahtarını gir, sonra üst bardan tarih ve lig seçip ANALİZİ BAŞLAT butonuna bas.""", unsafe_allow_html=True)else:indexed_fl = list(enumerate(fl))yuksek = [(idx, x) for idx, x in indexed_fl if x["t"]["ana_p"] >= 70]orta = [(idx, x) for idx, x in indexed_fl if 55 <= x["t"]["ana_p"] < 70]kombolu = [(idx, x) for idx, x in indexed_fl if x["t"].get("combo_var", False)]
