    ...
    if not API_KEY or not secili_kodlar:
        ...
        if not bulten.empty:
            ...
            for i, m in bulten.iterrows():
                ...
                if len(b) >= min_ornek:
                    ...
                    final_list.append({
                        ...
                        'ÖRNEK': len(b), 'idx': i, 'iy05_raw': iy05_p, 'ms15_raw': ms15_p, 'ms25_raw': ms25_p, 'kg_raw': kg_p, 'ms_vibe_raw': ms_mod,
            ...
            if final_list:
                ...
                st.dataframe(df_ana.drop(columns=['idx','iy05_raw','ms15_raw','ms25_raw','kg_raw','ms_vibe_raw','h_oran','b_oran','a_oran']).style.map(style_engine), use_container_width=True)
                ...
                for row in final_list:
                    with st.expander(f"🔍 {row['SAAT']} | {row['EV SAHİBİ']} vs {row['DEPLASMAN']}"):
                        ...
                        alt_t = "MS 1 & 1.5 ÜST" if row['ms_vibe_raw'] == 'H' and row['ms25_raw'] > 0.60 else "2.5 ÜST"
                        alt_o = (row['h_oran'] * 1.35) if row['ms_vibe_raw'] == 'H' else 1.65
                        ...
                        dt_res['MS_35'] = (b_det['FTHG'] + b_det['FTAG'] >= 4).map({True:'Over', False:'Under'})
                        ...
                        st.dataframe(dt_res.style.map(style_engine, subset=['1Y_05','İY_15','MS_15','MS_25','MS_35','KG_V','1Y_V','MS_V']), use_container_width=True, hide_index=True)
