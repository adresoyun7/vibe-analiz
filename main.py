...
APP_SCHEMA_VERSION = 20if st.session_state.get("app_schema_version") != APP_SCHEMA_VERSION:st.session_state.clear()st.session_state["app_schema_version"] = APP_SCHEMA_VERSION
...
def kombo_tahmini_oran(label, ana_odd=None):"""Top 10 Market içinde kombo marketler için yaklaşık oran üretir.Gerçek bookmaker kombo oranı API'den gelmediği için sadece tahmini gösterim amaçlıdır."""if not label:return None
...
def mac_key(m):"""Aynı maç + aynı market tekrarını engellemek için güvenli maç anahtarı."""try:if not isinstance(m, dict):try:m = m.to_dict()except Exception:m = {}
...
def gunun_en_iyi_10_uret(gecmis_df, bulten_df, min_ornek=1, limit=10):"""Top10 / Top50 özel liste üretici.
...
for key, default in [("final_list", []),("detay_idx", None),("detay_item", None),("top10_list", []),("top50_list", []),("filtre", "tumu"),("kupona", []),("coupon_popup_open", False),("last_gecmis_df", None),("last_bulten_df", None),]:if key not in st.session_state:st.session_state[key] = default
...
def tarih_secimine_gore_date(secim: str, bugun_tarih, ozel_tarih):if secim == "Bugün":return bugun_tarihif secim == "Yarın":return bugun_tarih + timedelta(days=1)if secim == "2 gün sonra":return bugun_tarih + timedelta(days=2)return ozel_tarih
...
bugun = datetime.now().date()API_KEY = get_app_api_key()

st.markdown("""

""", unsafe_allow_html=True)

def clear_detail_on_filter_change():st.session_state.detay_idx = Nonest.session_state.detay_item = None

def clear_detail_and_rebuild_top_markets():# Market filtresi değişince eski detay popup'ı açık kalmasın.st.session_state.detay_idx = Nonest.session_state.detay_item = None
...
if 'date_mode' not in st.session_state:st.session_state['date_mode'] = 'Bugün'if 'special_date' not in st.session_state:st.session_state['special_date'] = bugunif st.session_state.get('date_mode') == '3 gün sonra':st.session_state['date_mode'] = 'Özel Tarih'st.session_state['special_date'] = bugun + timedelta(days=3)

FİLTRELER ARTIK SOL SIDEBAR İÇİNDE

with st.sidebar:st.markdown("""⚡VIBE PRO""", unsafe_allow_html=True)with st.expander("🔑 API Key", expanded=False):current_key = st.session_state.get("user_api_key", "")api_key_input = st.text_input("ODDS API KEY", value=current_key, placeholder="API key gir...", type="password", key="api_key_input_sidebar_clean")a1, a2 = st.columns(2)with a1:if st.button("Kaydet", use_container_width=True, key="save_api_key_sidebar_clean"):st.session_state["user_api_key"] = api_key_input.strip()st.success("API Key kaydedildi ✅")st.rerun()with a2:if st.button("Temizle", use_container_width=True, key="clear_api_key_sidebar_clean"):st.session_state.pop("user_api_key", None)st.success("API Key temizlendi")st.rerun()if get_app_api_key():st.success("API key aktif ✅")else:st.warning("Maçları çekmek için API key girmen gerekiyor.")
...
def detay_popup_icerigi():if st.session_state.detay_item is not None:item = st.session_state.detay_itemelse:idx = st.session_state.detay_idxitem = st.session_state.final_list[idx]m, t, b_det = item["m"], item["t"], item["b"]
...
if st.session_state.detay_item is not None or st.session_state.detay_idx is not None:try:@st.dialog("Maç Detayı", width="large")def _detay_modal():detay_popup_icerigi()_detay_modal()except Exception:# Eski Streamlit sürümlerinde st.dialog yoksa detay yine sayfanın üstünde gösterilir.with st.container(border=True):detay_popup_icerigi()
...
if not fl:st.markdown("""⚡Analizi BaşlatınSol menüden API key ve filtreleri ayarla, sonra ANALİZİ BAŞLAT butonuna bas.""", unsafe_allow_html=True)else:indexed_fl = list(enumerate(fl))yuksek = [(idx, x) for idx, x in indexed_fl if x["t"]["ana_p"] >= 70]orta = [(idx, x) for idx, x in indexed_fl if 55 <= x["t"]["ana_p"] < 70]kombolu = [(idx, x) for idx, x in indexed_fl if x["t"].get("combo_var", False)]
