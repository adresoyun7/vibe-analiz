...
def style_engine(val):
    ...
    val_str = str(val)
    if any(x in val_str for x in ['Over', 'Yes', 'Home', '1/1', '2/2', '1/2', '2/1']): return 'background-color: #27ae60; color: white;'
    if any(x in val_str for x in ['Under', 'No', 'Away']): return 'background-color: #c0392b; color: white;'
    if any(x in val_str for x in ['Draw', 'Tie', 'Beraberlik', 'X/']): return 'background-color: #f39c12; color: white;'
    return ''
    ...
    if not API_KEY or not secili_kodlar:
        ...
        if not bulten.empty:
            ...
            for i, m in bulten.iterrows():
                ...
                if len(b) >= 1:
                    ...
                    final_list.append({
                        ...
                        '1Y_V': f"{iy_mod.replace('H','Ev').replace('A','Dep').replace('D','Ber')}",
            ...
            if final_list:
                ...
                st.markdown("---")
                st.subheader("📚 Detaylı Geçmiş & Tahmin Raporu")
                for row in final_list:
                    with st.expander(f"🔍 {row['SAAT']} | {row['EV SAHİBİ']} vs {row['DEPLASMAN']}"):
                        ...
                        st.info(f"🎯 **ANA TERCİH:** {'2.5 ÜST' if row['ms25_r'] > 0.65 else 'KG VAR' if row['kg_r'] > 0.6 else row['MS_V']} | "
                                f"🥈 **KOMBO:** {row['MS_V']} & {'KG VAR' if row['kg_r'] > 0.55 else 'KG YOK'} | "
                                f"🔥 **HT/FT:** {(gecmis.loc[gecmis.index.isin(b.index), 'HTR'] + '/' + gecmis.loc[gecmis.index.isin(b.index), 'FTR']).mode()[0].replace('H','1').replace('A','2').replace('D','X')}")
                        
                        if row['ms25_r'] > 0.65 and row['kg_r'] < 0.50:
                            st.warning("⚠️ TEK TARAFLI MAÇ RİSKİ: 3-0 veya 4-0 bitebilir, KG riskli!")
                        ...
                        if flip_p >= 0.10:
                            st.error(f"🔥 SÜRPRİZ RADARI: Bu oranda %{int(flip_p*100)} ihtimalle HT/FT sürprizi (1/2-2/1) saptandı!")
