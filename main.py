...
BASKETBOL_LIGLERI = {
    ...
    "🇪🇺 AVRUPA": {'Türkiye BSL': 'basketball_turkey_bsl', 'İspanya ACB': 'basketball_spain_liga_endesa', 'İtalya Lega A': 'basketball_italy_lega_a', 'Almanya BBL': 'basketball_germany_bbl', 'Fransa LNB': 'basketball_france_lnb', 'Yunanistan GBL': 'basketball_greece_basket_league'}
}
        ...
        if "Futbol" in spor_turu:
            ...
            if not bulten.empty:
                ...
                if final_list:
                    ...
                    st.download_button("📥 Excel İndir", to_excel(df.drop(columns=['idx'])), "Vibe.xlsx")

                    # AI RAPORU (Hatasız Çağırma)
                    st.markdown("### 🤖 Gemini AI Analizi")
                    try:
                        model = genai.GenerativeModel('gemini-1.5-flash')
                        response = model.generate_content(f"Futbol analisti gibi bu tabloyu yorumla: {df.to_string()}")
                        st.info(response.text)
                    except Exception as e: st.warning(f"AI bağlantı kuramadı: {e}")

                    if flips:
                        st.subheader("🔥 Sürpriz Radarı")
                        for f in flips: st.warning(f"{f['m']}: %{f['p']} Sürpriz!")
            else: st.error("Bültende maç bulunamadı.")
